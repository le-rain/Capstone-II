...
function [section_indices] = identifySectionsTM(istart, istop)
    ...
    for i = 1:(length(istart) - 1)
        if abs(istop(i) - istart(i+1)) < 15 %if difference between end of one wave and start of the next wave is small enough
    ...
    for i = 2:(length(consec) - 1) %starts at 2 to ignore first row of consec [0 0]
        if abs(consec(i,2) - consec(i+1,1)) > 15 %if difference between end of one wave and start of next wave is large
    ...
    for i = 2:(length(consec) - 1) 
        ...
        if abs(consec(i,2) - consec(i+1,1)) > 15
