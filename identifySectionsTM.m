%accepts sorted istart and istop vectors from findsignal function
...
function [section_indices] = identifySectionsTM(istart, istop)
    ...
    for i = 2:(length(consec) - 1) %starts at 2 to ignore first row [0 0]
        if abs(consec(i,2) - consec(i+1,1)) > 10
            num_sections = num_sections + 1; %number of individual sections found
        end
    end
    
    %split consecutive good waveforms into different cells
    sections = cell(num_sections,1); %preallocate cell array 
    j = 1; %initialize counter for sections number
    ...
    section_indices = zeros(num_sections,2)
