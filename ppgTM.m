...
%uses TM to indentify best sections of signal 
function [ppg_tm, time, frameRate, channel, template_type, template_resampled, template_resampled_rev, i, j] = ppgTM(file, roiHeight, roiWidth)
    ...
    ppg_tm = videoToPPGTM(video,channel,i,j,roiHeight,roiWidth,frameRate); %plot PPG based on best section from best channel
    time = plotPPGTM(fileName,ppg_tm,frameRate,channel,roiHeight,roiWidth); %plot PPG
